...
// ´ÓRTCÄÚ´æ»Ö¸´À¶ÑÀ×´Ì¬
void restore_ble_state(void) {
    esp_err_t err;
    err = bluetooth_init();
    if (err) {
        ESP_RTC_LOGI("esp32_bluetooth_init failed (err %d)", err);
        return;
    }
//    ble_mesh_get_dev_uuid(dev_uuid);
    ...
    err = ble_mesh_init();
    if (err) {
        ESP_RTC_LOGI( "Bluetooth mesh init failed (err %d)", err);
    }
}
...
void wake_stub_example(void)
{
    ...
    //example_ble_mesh_send_vendor_message(true,4,(uint8_t *)buf);
