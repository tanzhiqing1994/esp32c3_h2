...
extern esp_ble_mesh_generic_server_cb_param_t generic_param;
...
void ble_mesh_wake_up_handler(void)
{
    ESP_LOGI(TAG, "Device woke up from deep sleep");
    // 在这里添加唤醒后的处理逻辑
    // 例如：重新初始化BLE Mesh节点、OnOff Server等
}
                                               ...
                                               esp_ble_mesh_generic_server_cb_param_t *param)
{
    ...
    ESP_LOGI(TAG, "event 0x%02x, opcode 0x%04" PRIx32 ", src 0x%04x, dst 0x%04x",
        ...
        generic_param.model = param->model;
    switch (event) {
